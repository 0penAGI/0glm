...
class MultiNavigator(nn.Module):
    ...
    def __init__(self, feat_dim=FEAT_DIM, state_dim=STATE_DIM, hidden=HIDDEN_DIM,
        ...
        # BAKE: условие темой вопроса (384) — планировщик ведёт траекторию вопроса
        self.qsem_proj = nn.Linear(SEM_DIM, hidden)
        # BAKE v2: токен цели для трансформера (виден вниманию на каждом шаге)
        self.qsem_tok = nn.Linear(SEM_DIM, hidden)

    def forward(self, states, stream_idx=0, cond=None, return_hidden=False, qsem=None):
        B, K, _ = states.shape
        z = self.proj(self.feat_enc(states)) + self.pos[:, :K, :]
        # BAKE v2: вопрос как ОТДЕЛЬНЫЙ ТОКЕН в трансформере — внимание видит
        # цель на каждом шаге контекста (одноразовый укол в h разбавлялся 1:3
        # и поле стартовало мимо темы: pred·q=.25 на чёрных дырах)
        q_last = None
        if qsem is not None:
            z = torch.cat([z, self.qsem_tok(qsem).unsqueeze(1)], dim=1)
            q_last = True
        z = self.transformer(z)
        ctx_z = z[:, K - 1, :] if q_last else z[:, -1, :]
        ...
        if qsem is not None:
            h = h + self.qsem_proj(qsem)

        heads = (self.stream_cluster(h), self.stream_level(h),
                 ...
                 self.affect_head(h))
        return heads + (h,) if return_hidden else heads
...
def build_training_pairs(pool, clusters, sems=None, proj_all=None, affect_all=None,
    ...
    for traj in pool.get("trajectories", []):
        ...
        # BAKE (запекание планировщика): отпечаток темы всей траектории — среднее
        # сем первых зёрен, нормализованное. Условие «иди в этой теме» для КАЖДОЙ
        # траектории, не только QA-доков: поле учится вести вопросом.
        qsem = None
        if sems is not None:
            _head = traj[:16]
            _vs = [sems[int(l)][int(i)] for l, i in _head
                   if int(l) <= 2 and i < len(sems[int(l)])]
            if _vs:
                _m = np.mean(_vs, axis=0)
                _n = float(np.linalg.norm(_m))
                if _n > 1e-6:
                    qsem = (_m / _n).astype(np.float16)
...
class MultiPairDS(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (torch.tensor(p["ctx"]), torch.tensor(p["cluster"]),
                ...
                torch.tensor(p.get("qsem", self.z16), dtype=torch.float32))
...
def train_multi(model, pairs, n_steps=TRAIN_STEPS):
    ...
    while step < n_steps:
        for batch in loader:
            if step >= n_steps: break
            ctx, tgt_c, tgt_l, tgt_p, tgt_dn, tgt_d, cond, stream_idx, tgt_sem, tgt_top, tgt_aff, qsem = \
                [b.to(DEVICE) for b in batch]
            cl, lv, pr, dn, zz, sem_pred, topic_pred, aff_pred = model(ctx, stream_idx=stream_idx,
                                                                      cond=cond, qsem=qsem)
...
# --- канонизация корпуса (директива 2026-08-25): синдикат физически кладёт ---
# --- одни и те же тексты в пул многократно с мелкими правками; это причина  ---
# --- дублей в выдаче, а не поведение генератора. Чистим ПАМЯТЬ раз и навсегда
def canonical_mask(pool, force=False):
    """Булева маска канонических зёрен по уровням. Два прохода:
    1) полный нормализованный хэш (lowercase, пунктуация→пробел) — точные копии;
    2) бакеты по первым 60 нормализованным символам + косинус сем >0.97 внутри
       бакета — синдикатные варианты с мелкими правками."""
    cache = str(POOL_DIR / "text_canon_v1.npz")
    lens = [len(pool["micro_texts"]), len(pool["meso_texts"]), len(pool["macro_texts"])]
    if not force and os.path.exists(cache):
        z = np.load(cache)
        if len(z["m_micro"]) == lens[0] and len(z["m_meso"]) == lens[1]:
            print(f"🧹 Canon cache: {int(z['m_micro'].sum())+int(z['m_meso'].sum())+int(z['m_macro'].sum())}"
                  f"/{sum(lens)} зёрен каноничны")
            return {0: z["m_micro"], 1: z["m_meso"], 2: z["m_macro"]}
    import re as _re
    sems_c = build_semantics(pool)
    masks = {}
    for lv in range(3):
        texts = pool[f"{['micro','meso','macro'][lv]}_texts"]
        m = np.ones(lens[lv], dtype=bool)
        seen_full, buckets = {}, {}
        for i, t in enumerate(texts):
            nrm = _re.sub(r"[^a-zа-я0-9]+", " ", t.lower()).strip()
            if not nrm or len(nrm) < 20:          # вырожденные обрывки
                m[i] = False
                continue
            h = hash(nrm)
            if h in seen_full:
            ...
            b = nrm[:60]
            dup = False
            for j in buckets.get(b, ()):
                if float(sems_c[lv][i] @ sems_c[lv][j]) > 0.97:
                    dup = True
                    break
            if dup:
                m[i] = False
        ...
        masks[lv] = m
        print(f"   🧹 lvl{lv}: {int(m.sum())}/{lens[lv]} каноничны")
    np.savez(cache, m_micro=masks[0], m_meso=masks[1], m_macro=masks[2])
    print(f"   ✅ canon → {cache}")
    return masks
